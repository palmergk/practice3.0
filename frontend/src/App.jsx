...
const App = () => {
  ...
  const UpdateUser = async () => {
    const endPoint = 'api/user/update-user'
    try {
      setLoading(true)
      const response = await axios.put(`${backendURL}/${endPoint}`, form) //api
      if (response.data.status === 200) {
        FetchUsers()
        SuccessAlert(response.data.msg)
        setForm({
  ...
  return (
    <div className='w-11/12 mx-auto py-10'>
      <div className='bg-white shadow-lg w-fit h-fit rounded-md mx-auto flex flex-col gap-4 p-10 relative'>
        ...
        {!form.user_id ? <button className='w-fit h-fit py-2 px-6 bg-[#d6d649] mx-auto rounded-md mt-4 font-medium' onClick={CreateUser}>Create</button>
          :
          <button className='w-fit h-fit py-2 px-6 bg-[#d6d649] mx-auto rounded-md mt-4 font-medium' onClick={UpdateUser}>Update</button>
        }
      </div>
      <UserComponent allUsers={allUsers} setAllUsers={setAllUsers} setForm={setForm} FetchUsers={FetchUsers} />
    </div>
  )
}
